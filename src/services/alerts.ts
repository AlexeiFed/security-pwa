...
export const subscribeActiveAlert = (callback: (alert: Alert | null) => void, user?: { uid: string; role: string }) => {
    ...
    activeAlertUnsubscribe = onSnapshot(q, (querySnapshot) => {
        ...
        // Воспроизводим звук только при появлении новой тревоги (но не для администраторов и не для инициатора тревоги)
        if (!activeAlert && latestActiveAlert) {
            // Не воспроизводим звук для администраторов и для инициатора тревоги
            if (user && user.role !== 'admin' && latestActiveAlert.userId !== user.uid) {
                console.log('Новая тревога получена, воспроизводим звук');
                playAlarm();
            } else {
                console.log('Новая тревога получена, звук не воспроизводится (администратор или инициатор тревоги)');
            }
